...
export const applyPrimitive = (proc: PrimOp, args: Value[]): Result<Value> =>
  proc.op === '+'
    ...
    : proc.op === '*'
    ? allT(isNumber, args)
      ? makeOk(reduce((x: number, y: number) => x * y, 1, args))
      : makeFailure('* expects numbers only')
    : proc.op === '/'
    ? divPrim(args)
    : proc.op === '>'
    ? allT(isNumber, args) || allT(isString, args)
      ? makeOk(args[0] > args[1])
      : makeFailure('> expects numbers or strings only')
    : proc.op === '<'
    ? allT(isNumber, args) || allT(isString, args)
      ? makeOk(args[0] < args[1])
      : makeFailure('< expects numbers or strings only')
...
const carPrim = (v: Value): Result<Value> =>
  isCompoundSExp(v) ? makeOk(v.val1) : makeFailure(`Car: param is not compound ${v}`)

const cdrPrim = (v: Value): Result<Value> =>
  isCompoundSExp(v) ? makeOk(v.val2) : makeFailure(`Cdr: param is not compound ${v}`)

const consPrim = (v1: Value, v2: Value): CompoundSExp => makeCompoundSExp(v1, v2)

export const listPrim = (vals: Value[]): EmptySExp | CompoundSExp =>
  vals.length === 0 ? makeEmptySExp() : makeCompoundSExp(first(vals), listPrim(rest(vals)))

const isListPrim = (v: Value): boolean => isEmptySExp(v) || isCompoundSExp(v)
